...
TorranceSparrow::TorranceSparrow(const Vector3d & f0, float roughness, Texture * texture): Material(texture) {
    this->alpha = roughness * roughness;
    this->f0 = f0;
}

Vector3d TorranceSparrow::getBRDF(const Vector3d & wi, const Vector3d & wo, const HitResult & hitResult) {
    Vector3d n = wo.dot(hitResult.n) < 0 ? -hitResult.n : hitResult.n;
    float cos_theta_i = -(wi.dot(n));
    float cos_theta_o = wo.dot(n);
    Vector3d wh = (wo - wi).normalize();
    return F(wo, wh) * G(wi, wo, n) * D(wh, n) / (4 * cos_theta_i * cos_theta_o);
}

Vector3d TorranceSparrow::F(const Vector3d & wo, const Vector3d & wh) {
    return f0 + (1 - f0) * (float) std::pow(1.f - wh.dot(wo), 5);
}

float TorranceSparrow::G(const Vector3d & wi, const Vector3d & wo, const Vector3d & n) const {
    float cos1 = -(wi.dot(n));
    float cos2 = wo.dot(n);
...
float TorranceSparrow::D(const Vector3d & wh, const Vector3d & n) const {
    float a2 = alpha * alpha;
    float cos = wh.dot(n);
...
Vector3d TorranceSparrow::sampleBRDF(const Vector3d & wo, float * pdf, const HitResult & hitResult) {
    ...
    // 转回原来的位置
    Vector3d wh(m1.dot(v), m2.dot(v), m3.dot(v));

    // 计算采样的 wi 方向
    float cos_o = wh.dot(wo);
    Vector3d wi = 2 * cos_o * wh - wo;
...
float TorranceSparrow::getPdf(const Vector3d & wi, const Vector3d & wo, const HitResult & hitResult) {
    Vector3d n = wo.dot(hitResult.n) < 0 ? -hitResult.n : hitResult.n;
    Vector3d wh = (wo - wi).normalize();
...
Vector3d TorranceSparrow::getEmitting(const Vector3d & wi, const HitResult & hitResult) {
    return {0, 0, 0};
}
